...
% Symmetric orbits of type AnBn
for counter=1:19
    k=k+1;
    filename = sprintf('../../Data/Lorenz/DATA3/orbit%d.dat',counter);
    orbit=load(filename);
    x0 = orbit(:,1);
    [~,xdat(:,:,k)] = ode45(@(t,x)LORENZ(t,x),t,x0,ode_options);
    filename = sprintf('../../Data/Lorenz/DATA3/T%d.dat',counter);
    PeriodUPO(k,1) =  load(filename);
    PeriodUPO(k,2) = ceil(PeriodUPO(k,1)/dt);

end

% Orbits of type AnB and BnA
for counter=1:23
    k=k+1;
    filename = sprintf('../../Data/Lorenz/DATA4/orbit%d.dat',counter);
    orbit=load(filename);
    x0 = orbit(:,1);
    [~,xdat(:,:,k)] = ode45(@(t,x)LORENZ(t,x),t,x0,ode_options);
    filename = sprintf('../../Data/Lorenz/DATA4/T%d.dat',counter);
    PeriodUPO(k,1) =  load(filename);
    PeriodUPO(k,2) = ceil(PeriodUPO(k,1)/dt);
    k=k+1;
    x0(1) = x0(1)*-1;
