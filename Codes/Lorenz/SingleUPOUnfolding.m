% Code to study the unfolding a single UPO 
% Written by: Prerna Patil 
% Date: 18th Jnauary 2024
...
%     close all
    ...
    for k = 5
        disp(k);
        clear H
        H(:,:) = CreateHankelMatrix(xdatPred(:,:,k),...
    ...
    if(saveLegend)
        ...
        legend(LegendNames,'Location','best','box','off','interpreter','tex','NumColumns',1);
            ...
            legend(UPOSequences,'Location','best','box','off','interpreter','tex','NumColumns',2);
        end
        xlabel('x'); ylabel('y');
